                    ...
                    ->action(function (RelationManager $livewire) {
                        try {
                            ...
                            $metaTitle = $prism->text()
                                ...
                                ->withPrompt("Write an SEO-optimized meta title under 60 characters for a digital marketing product or service named '{$category->name}'.")
                                ->asText();

                            $keywords = $prism->text()
                                ->using(Provider::OpenRouter, 'openai/gpt-oss-20b:free')
                                ->withPrompt("Generate 10–15 comma-separated SEO keywords for '{$category->name}'.")
                                ->asText();

                            $metaDescription = $prism->text()
                                ->using(Provider::OpenRouter, 'openai/gpt-oss-20b:free')
                                ->withPrompt("Write a compelling SEO meta description for '{$category->name}'.")
                                ->asText();

                            $meta = $category->metaDetails()->first();
